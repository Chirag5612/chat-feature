...
const addAllFirebaseUser = async (data) => {
    ...
    try {
        const docRef = doc(db, "ChatUsers", data._id);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
            await updateDoc(doc(db, "ChatUsers", data._id), newData, { capital: true }, { merge: true });

        } else {
            await setDoc(doc(db, "ChatUsers", data._id), newData, { capital: true }, { merge: true });
        }
